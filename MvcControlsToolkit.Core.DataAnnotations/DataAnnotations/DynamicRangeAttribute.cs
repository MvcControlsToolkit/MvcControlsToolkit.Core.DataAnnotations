    ...
    public class DynamicRangeAttribute : ValidationAttribute
    {
        ...
        public string SMinimum
        {
            set
            {
                ...
                try
                {
                    if (typeof(ITimeRestriction).IsAssignableFrom(TargetType)) Minimum = TimeRestrictionParser.Parse(TargetType, value);
        ...
        public string SMaximum
        {
            set
            {
                ...
                try
                {
                    if (typeof(ITimeRestriction).IsAssignableFrom(TargetType)) Maximum = TimeRestrictionParser.Parse(TargetType, value);
        ...
        private object getDelay(string delayRef, object model, object fixedDelay = null, bool invertSign = false)
        {
            ...
            {
                if (TargetType == typeof(DateTime) || TargetType == typeof(TimeSpan) || typeof(ITimeRestriction).IsAssignableFrom(TargetType)) fixedDelay = TimeSpan.Parse(delayRef.Substring(1), CultureInfo.InvariantCulture);
            ...
            if ((TargetType == typeof(DateTime) || TargetType == typeof(TimeSpan) || typeof(ITimeRestriction).IsAssignableFrom(TargetType)) && res != null) return Convert.ToInt64(((TimeSpan)res).TotalMilliseconds);
            return res;
        }
        private object changeSign(object value)
        {
            ...
            else if (TargetType == typeof(DateTime) || TargetType == typeof(DateTimeOffset) || TargetType == typeof(TimeSpan) || typeof(ITimeRestriction).IsAssignableFrom(TargetType))
            {
        ...
        private object addDelay(string delayRef, ref IComparable value, object model, object fixedDelay = null, bool subtract = false)
        {
            if (delayRef != null && delayRef.StartsWith("!"))
            {
                if (TargetType == typeof(DateTime) || TargetType == typeof(DateTimeOffset) || typeof(ITimeRestriction).IsAssignableFrom(TargetType)) fixedDelay = TimeSpan.Parse(delayRef.Substring(1), CultureInfo.InvariantCulture);
                else fixedDelay = Convert.ChangeType(delayRef.Substring(1), TargetType, CultureInfo.InvariantCulture);
            }
            ...
            if (subtract)
            {
                ...
                else if (typeof(ITimeRestriction).IsAssignableFrom(TargetType))
                {
                    value = (value as ITimeRestriction).Add(-(TimeSpan)toAdd);
                    toAdd = -Convert.ToInt64(((TimeSpan)toAdd).TotalMilliseconds);
                }
        ...
        public override string FormatErrorMessage(string name)
        {
            ...
            cmin = (cmin != null && typeof(ITimeRestriction).IsAssignableFrom(TargetType) ) ? ((ITimeRestriction)cmin).ToDateTime() : cmin;
            cmax = (cmax != null && typeof(ITimeRestriction).IsAssignableFrom(TargetType)) ? ((ITimeRestriction)cmax).ToDateTime() : cmax;
            if (cmin != null)
            {
